...
export interface Course {
  ...
  subjectRequirements?: Record<
    string,
    number | { alternatives: { subject: string; level: number }[] }
  >;
  duration?: string;
}
...
export abstract class BaseUniversity {
   ...
   * Always pass raw subject percentages (0-100); implementations convert internally.
   */
  abstract calculateApsScore(subjects: Record<string, number>): number;
  ...
  getExtendedCurriculumPrograms(): DetailedCourse[] {
    return this._courses.filter((course) => {
      ...
      const nameMatch =
        course.name.toLowerCase().includes("extended curriculum") ||
      ...
      const requirementsMatch =
        typeof course.additionalRequirements === "string"
          ? course.additionalRequirements.toLowerCase().includes("extended")
          : Array.isArray(course.additionalRequirements)
            ? course.additionalRequirements.some((req) =>
                req.toLowerCase().includes("extended"),
              )
            : false;
