...
import { modelConfig } from "@/lib/model-config" // Import updated modelConfig
...
describe("Model Configuration", () => {
  ...
  it("should return specific model config", () => {
    const config = modelConfig.getModelConfig("gemini-1.5-flash")

    expect(config.name).toBe("gemini-1.5-flash")
    expect(config.displayName).toContain("Gemini")
  })

  it("should throw error for invalid model", () => {
    expect(() => {
      modelConfig.getModelConfig("invalid-model" as any)
    }).toThrow(ModelConfigError)
  })
